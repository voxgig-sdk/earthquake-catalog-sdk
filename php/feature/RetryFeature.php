...
// EarthquakeCatalog SDK retry feature
...
class EarthquakeCatalogRetryFeature extends EarthquakeCatalogBaseFeature
{
    ...
    public function init(EarthquakeCatalogContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EarthquakeCatalogContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EarthquakeCatalogContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EarthquakeCatalogContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
