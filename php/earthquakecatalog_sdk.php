...
class EarthquakeCatalogSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = EarthquakeCatalogHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = EarthquakeCatalogHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!EarthquakeCatalogFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, EarthquakeCatalogFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return EarthquakeCatalogUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = EarthquakeCatalogHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = EarthquakeCatalogHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = EarthquakeCatalogHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new EarthquakeCatalogError($op . "_allow",
                "EarthquakeCatalogSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = EarthquakeCatalogHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new EarthquakeCatalogError("graphql_error",
                "EarthquakeCatalogSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function EarthquakeData($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_earthquake_data === null) {
                $this->_earthquake_data = new EarthquakeDataEntity($this, null);
            }
            return $this->_earthquake_data;
        }
        return new EarthquakeDataEntity($this, $data);
    }
    ...
    public function ServiceInformation($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_service_information === null) {
                $this->_service_information = new ServiceInformationEntity($this, null);
            }
            return $this->_service_information;
        }
        return new ServiceInformationEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new EarthquakeCatalogSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
