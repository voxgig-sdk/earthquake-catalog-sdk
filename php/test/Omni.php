...
//    only - a typed EarthquakeCatalogContext there would make every ctx
...
final class EarthquakeCatalogOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new EarthquakeCatalogContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new EarthquakeCatalogSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new EarthquakeCatalogResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new EarthquakeCatalogError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new EarthquakeCatalogResponse($clean['response']);
    ...
    public static function err_from_map($m): ?EarthquakeCatalogError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new EarthquakeCatalogError(is_string($code) ? $code : '', $msg);
    }
