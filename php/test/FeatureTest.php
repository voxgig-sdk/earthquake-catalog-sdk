...
// EarthquakeCatalog SDK feature test
//
...
class FtCtrl extends EarthquakeCatalogControl
{
...
class FtClient
{
    ...
    private EarthquakeCatalogUtility $utility;

    public function __construct(EarthquakeCatalogUtility $utility, array $options)
    {
    ...
    public function get_utility(): EarthquakeCatalogUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EarthquakeCatalogContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EarthquakeCatalogContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EarthquakeCatalogContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EarthquakeCatalogConfig::make_feature($name);
    ...
    private static function buildUrl(EarthquakeCatalogSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EarthquakeCatalogError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EarthquakeCatalogResponse($fetched);
        }
    ...
    private function fail(EarthquakeCatalogContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EarthquakeCatalogContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
