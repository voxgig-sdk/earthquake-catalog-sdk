...
The PHP SDK for the EarthquakeCatalog API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->EarthquakeData(null)->list(null, null);
...
```php
[$result, $err] = $client->EarthquakeData(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->EarthquakeCatalog(null)->load(
    ["id" => "test01"], null
);
