...
The PHP SDK for the EarthquakeCatalog API — an entity-oriented client using PHP conventions.
...
try {
    ...
    foreach ($earthquakedatas as $record) {
...
try {
    ...
    print_r($earthquakedata->data_get());
} catch (\Throwable $err) {
    echo "Error: " . $err->getMessage();
}
```
...
print_r(array_map(fn($item) => $item->data_get(), $earthquakedata));
```
...
```
php/
├── earthquakecatalog_sdk.php          -- Main SDK class
