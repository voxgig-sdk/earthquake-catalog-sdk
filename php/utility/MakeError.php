...
class EarthquakeCatalogMakeError
{
    public static function call(?EarthquakeCatalogContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EarthquakeCatalogContext([], null);
        }
        $op = $ctx->op ?? new EarthquakeCatalogOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EarthquakeCatalogResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EarthquakeCatalogError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new EarthquakeCatalogError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof EarthquakeCatalogError) {
