...
// EarthquakeCatalog SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = EarthquakeCatalogOmni::makeStructRunner(
    ...
    {
        return EarthquakeCatalogOmni::objmodel(EarthquakeCatalogOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return EarthquakeCatalogOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return EarthquakeCatalogOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(EarthquakeCatalogOmni::structfix($now, Struct::undef()))
                        !== json_encode(EarthquakeCatalogOmni::structfix($val, Struct::undef()))) {
