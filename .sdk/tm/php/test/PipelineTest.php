...
// EarthquakeCatalog SDK pipeline test
//
...
{
    private static function utility(): EarthquakeCatalogUtility
    {
        return new EarthquakeCatalogUtility();
    }
    ...
    private static function ctx(array $over = []): EarthquakeCatalogContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EarthquakeCatalogContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EarthquakeCatalogOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EarthquakeCatalogError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EarthquakeCatalogOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EarthquakeCatalogMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EarthquakeCatalogOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EarthquakeCatalogMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EarthquakeCatalogOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EarthquakeCatalogMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EarthquakeCatalogSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EarthquakeCatalogMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new EarthquakeCatalogResponse([]),
            'result' => new EarthquakeCatalogResult([]),
        ]);
        [, $err] = EarthquakeCatalogMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EarthquakeCatalogSpec([]),
            'response' => null,
            'result' => new EarthquakeCatalogResult([]),
        ]);
        [, $err] = EarthquakeCatalogMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EarthquakeCatalogSpec(['step' => 's']),
            'response' => new EarthquakeCatalogResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EarthquakeCatalogResult([]),
        ]);
        [, $err] = EarthquakeCatalogMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EarthquakeCatalogSpec(['step' => 's']),
            'response' => new EarthquakeCatalogResponse(self::resp(200, ['v' => 1])),
            'result' => new EarthquakeCatalogResult([]),
        ]);
        [, $err] = EarthquakeCatalogMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EarthquakeCatalogSpec(['step' => 's']),
            'response' => new EarthquakeCatalogResponse(self::resp(200, ['v' => 2])),
            'result' => new EarthquakeCatalogResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EarthquakeCatalogMakeResponse::call($ctx);
    ...
    {
        $preset = new EarthquakeCatalogResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EarthquakeCatalogSpec([]),
            'response' => new EarthquakeCatalogResponse([]),
            'result' => new EarthquakeCatalogResult([]),
        ...
        [$got, $err] = EarthquakeCatalogMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EarthquakeCatalogResult([])]);
        [, $err] = EarthquakeCatalogMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EarthquakeCatalogSpec([]), 'result' => null]);
        [, $err] = EarthquakeCatalogMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EarthquakeCatalogSpec(['step' => 's']),
            'result' => new EarthquakeCatalogResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EarthquakeCatalogOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EarthquakeCatalogMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EarthquakeCatalogSpec(['step' => 's']),
            'result' => new EarthquakeCatalogResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EarthquakeCatalogOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EarthquakeCatalogMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EarthquakeCatalogResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EarthquakeCatalogSpec([]),
            'result' => new EarthquakeCatalogResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EarthquakeCatalogMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EarthquakeCatalogMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EarthquakeCatalogError('boom', 'boom');
        $utility->fetcher = function (EarthquakeCatalogContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EarthquakeCatalogSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EarthquakeCatalogMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EarthquakeCatalogContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EarthquakeCatalogMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EarthquakeCatalogResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        EarthquakeCatalogMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EarthquakeCatalogContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EarthquakeCatalogSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EarthquakeCatalogMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EarthquakeCatalogResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EarthquakeCatalogSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EarthquakeCatalogMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EarthquakeCatalogMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EarthquakeCatalogMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EarthquakeCatalogResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EarthquakeCatalogDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EarthquakeCatalogResult(['ok' => false])]);
        $this->expectException(EarthquakeCatalogError::class);
        EarthquakeCatalogDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EarthquakeCatalogResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EarthquakeCatalogMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EarthquakeCatalogMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EarthquakeCatalogBaseFeature();
        $b = new EarthquakeCatalogBaseFeature();
        EarthquakeCatalogFeatureAdd::call($ctx, $a);
        EarthquakeCatalogFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EarthquakeCatalogBaseFeature
    {
        $f = new EarthquakeCatalogBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EarthquakeCatalogFeatureAdd::call($ctx, self::named_feature('a'));
        EarthquakeCatalogFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        EarthquakeCatalogFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        EarthquakeCatalogFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        EarthquakeCatalogFeatureAdd::call($ctx, $replace);
        ...
        EarthquakeCatalogFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?EarthquakeCatalogSpec $spec): EarthquakeCatalogContext
    {
    ...
    private static function auth_bags(): EarthquakeCatalogSpec
    {
        return new EarthquakeCatalogSpec(['headers' => [], 'query' => []]);
    }
    ...
    private static function auth_probe(array $options): ?array
    {
        ...
        EarthquakeCatalogPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EarthquakeCatalogPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EarthquakeCatalogResponse(['headers' => null]),
            'result' => new EarthquakeCatalogResult([]),
        ]);
        EarthquakeCatalogResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        EarthquakeCatalogResultBody::call($ctx);
