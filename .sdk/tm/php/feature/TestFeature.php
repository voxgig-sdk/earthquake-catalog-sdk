...
// EarthquakeCatalog SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class EarthquakeCatalogTestFeature extends EarthquakeCatalogBaseFeature
{
    ...
    public function init(EarthquakeCatalogContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (EarthquakeCatalogContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = EarthquakeCatalogParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (EarthquakeCatalogContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(EarthquakeCatalogContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = EarthquakeCatalogParam::call($ctx, $k);
