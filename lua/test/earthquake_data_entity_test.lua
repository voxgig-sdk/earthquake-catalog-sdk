...
describe("EarthquakeDataEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:EarthquakeData(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:EarthquakeData(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:EarthquakeData(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = earthquake_data_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "earthquake_data." .. _op, _live and "live" or "unit")
    ...
    local earthquake_data_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.earthquake_data")))
    local earthquake_data_ref01_data = nil
    if #earthquake_data_ref01_data_raw > 0 then
      earthquake_data_ref01_data = helpers.to_map(earthquake_data_ref01_data_raw[1][2])
    end

    -- LIST
    local earthquake_data_ref01_ent = client:EarthquakeData(nil)
    local earthquake_data_ref01_match = {}

    local earthquake_data_ref01_list_result, err = earthquake_data_ref01_ent:list(earthquake_data_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(earthquake_data_ref01_list_result)

    -- LOAD
    local earthquake_data_ref01_match_dt0 = {
...
function earthquake_data_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/earthquake_data/EarthquakeDataTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read earthquake_data test data: " .. entity_data_file)
  end
