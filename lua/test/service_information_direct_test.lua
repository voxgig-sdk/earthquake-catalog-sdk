-- ServiceInformation direct test
...
describe("ServiceInformationDirect", function()
  it("should direct-list-service_information", function()
    local setup = service_information_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-service_information", setup.live and "live" or "unit")
  ...
  it("should direct-load-service_information", function()
    local setup = service_information_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-service_information", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function service_information_direct_setup(mockres)
