# EarthquakeCatalog Lua SDK



The Lua SDK for the EarthquakeCatalog API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:EarthquakeData()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local service_information, err = client:ServiceInformation():load()
```
...
```
lua/
├── earthquake-catalog_sdk.lua    -- Main SDK module
...
The main module (`earthquake-catalog_sdk`) exports the SDK constructor
