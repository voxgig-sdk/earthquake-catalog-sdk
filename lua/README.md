...
The Lua SDK for the EarthquakeCatalog API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:EarthquakeData(nil):list(nil, nil)
...
```lua
local result, err = client:EarthquakeData(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:EarthquakeCatalog(nil):load(
  { id = "test01" }, nil
)
