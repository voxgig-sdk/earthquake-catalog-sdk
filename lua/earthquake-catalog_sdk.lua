...
require("earthquake-catalog_types")
...
local EarthquakeCatalogSDK = {}
EarthquakeCatalogSDK.__index = EarthquakeCatalogSDK
...
EarthquakeCatalogSDK._make_feature = _make_feature


function EarthquakeCatalogSDK.new(options)
  local self = setmetatable({}, EarthquakeCatalogSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function EarthquakeCatalogSDK:options_map()
...
function EarthquakeCatalogSDK:get_utility()
  return Utility.copy(self._utility)
end


function EarthquakeCatalogSDK:get_root_ctx()
  return self._rootctx
end


function EarthquakeCatalogSDK:prepare(fetchargs)
...
function EarthquakeCatalogSDK:direct(fetchargs)
...
function EarthquakeCatalogSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function EarthquakeCatalogSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "EarthquakeCatalogSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function EarthquakeCatalogSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function EarthquakeCatalogSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "EarthquakeCatalogSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function EarthquakeCatalogSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = EarthquakeCatalogSDK.new(sdkopts)
  sdk.mode = "test"
