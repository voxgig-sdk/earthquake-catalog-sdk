# EarthquakeCatalog TypeScript SDK



The TypeScript SDK for the EarthquakeCatalog API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.EarthquakeData()` — each with a small set of operations (`list`, `load`)
...
```ts
try {
  const earthquakedata = await client.EarthquakeData().load()
  console.log(earthquakedata)
} catch (err) {
...
| `tester(testopts?, sdkopts?)` | `EarthquakeCatalogSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `EarthquakeCatalogSDK.test(testopts?, sdkopts?)` | `EarthquakeCatalogSDK` | Create a test-mode client. |
...
| `client` | `client(): EarthquakeCatalogSDK` | Return the parent SDK client. |
...
```ts
const earthquake_data = await client.EarthquakeData().load()
```
...
```ts
const service_information = await client.ServiceInformation().load()
```
...
earthquake-catalog/
├── src/
│   ├── EarthquakeCatalogSDK.ts        # Main SDK class
