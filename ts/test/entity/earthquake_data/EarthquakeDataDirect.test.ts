...
describe('EarthquakeDataDirect', async () => {
  ...
  test('direct-load-earthquake_data', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-earthquake_data', setup.live)) return
  ...
  test('direct-list-earthquake_data', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-earthquake_data', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['EARTHQUAKE_CATALOG_TEST_EARTHQUAKE_DATA_ENTID']
