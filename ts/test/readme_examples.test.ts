...
//     (\`new EarthquakeCatalogSDK(...)\` / \`EarthquakeCatalogSDK.test(...)\` become
//     \`EarthquakeCatalogSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'EarthquakeCatalogSDK'
...
const TEST_SEED = {"entity":{"earthquake_data":{"test01":{"id":"test01"}},"service_information":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EarthquakeCatalogSDK(...)\` and any
// \`EarthquakeCatalogSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EarthquakeCatalogSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EarthquakeCatalogSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
