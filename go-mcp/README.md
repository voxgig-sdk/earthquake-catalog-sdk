# earthquake-catalog-mcp

[MCP](https://modelcontextprotocol.io) server exposing the EarthquakeCatalog SDK as
two agent tools — `earthquake-catalog_list` and `earthquake-catalog_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/earthquake-catalog-mcp)
make build

# 2. Provide credentials via the environment
export EARTHQUAKE_CATALOG_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user earthquake-catalog \
  -- /absolute/path/to/earthquake-catalog-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./earthquake-catalog-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// earthquake-catalog_list: first page of records
{ "entity": "earthquake_data" }
{ "entity": "earthquake_data", "query": { } }

// earthquake-catalog_load: one record by id
{ "entity": "earthquake_data", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/earthquake-catalog-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user earthquake-catalog \
     -- "$PWD"/dist/*/earthquake-catalog-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `earthquake-catalog_list` and `earthquake-catalog_load` tools now appear
   in new sessions. Ask the agent to *"list earthquake_data using earthquake-catalog"*
   and it calls `earthquake-catalog_list` with `{"entity":"earthquake_data"}`.
...
```sh
export EARTHQUAKE_CATALOG_APIKEY=sk_live_xxx            # API key
export EARTHQUAKE_CATALOG_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `earthquake-catalog_list` tool
...
### Call the `earthquake-catalog_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `earthquake-catalog_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `earthquake-catalog_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EARTHQUAKE_CATALOG_APIKEY` | API key sent with every request. |
| `EARTHQUAKE_CATALOG_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

earthquake_data | service_information

### Smoke test via HTTP (raw JSON-RPC)

```sh
./earthquake-catalog-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"earthquake-catalog_load","arguments":{"entity":"earthquake_data","query":{"id":1}}}}'
```
