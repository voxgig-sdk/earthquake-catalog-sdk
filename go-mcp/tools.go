...
type Args struct {
	Entity string         `json:"entity" jsonschema:"earthquake_data | service_information"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.EarthquakeCatalogSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "earthquake-catalog_list",
		Description: "List records from EarthquakeCatalog. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "earthquake-catalog_load",
		Description: "Load a single record from EarthquakeCatalog. " +
...
func runOp(client *sdk.EarthquakeCatalogSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.EarthquakeCatalogSDK, name string) (sdk.EarthquakeCatalogEntity, error) {
	switch strings.ToLower(name) {
	case "earthquake_data":
		return client.EarthquakeData(nil), nil
	case "service_information":
		return client.ServiceInformation(nil), nil
