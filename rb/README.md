...
The Ruby SDK for the EarthquakeCatalog API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.EarthquakeData(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.EarthquakeCatalog(nil).load(
  { "id" => "test01" }, nil
)
