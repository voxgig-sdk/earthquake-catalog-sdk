# EarthquakeCatalog Ruby SDK



The Ruby SDK for the EarthquakeCatalog API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.EarthquakeData` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
raise a `EarthquakeCatalogError` (a `StandardError` subclass), so wrap
...
```
rb/
├── EarthquakeCatalog_sdk.rb       -- Main SDK module
...
The main module (`EarthquakeCatalog_sdk`) exports the SDK class
...
# earthquakedata.data_get now returns the earthquakedata data from the last list
