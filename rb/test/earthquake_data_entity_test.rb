...
class EarthquakeDataEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EARTHQUAKE_CATALOG_TEST_EARTHQUAKE_DATA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    earthquake_data_ref01_data_dt0_load_result = Helpers.to_map(earthquake_data_ref01_data_dt0_loaded.respond_to?(:data_get) ? earthquake_data_ref01_data_dt0_loaded.data_get : earthquake_data_ref01_data_dt0_loaded)
...
def earthquake_data_basic_setup(extra)
  ...
  entid_env_raw = ENV["EARTHQUAKE_CATALOG_TEST_EARTHQUAKE_DATA_ENTID"]
