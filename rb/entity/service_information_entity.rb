...
class ServiceInformationEntity
  ...
  def make
    opts = @_entopts.dup
    ServiceInformationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = EarthquakeCatalogHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ServiceInformation, Hash] the current ServiceInformation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EarthquakeCatalogHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ServiceInformation fields)
  def match_get
  ...
  #
  # @param reqmatch [ServiceInformationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ServiceInformation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ServiceInformation, Hash] the loaded ServiceInformation; raises EarthquakeCatalogError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EarthquakeCatalogHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ServiceInformationListMatch, Hash, nil] match filter (any subset of
  #   ServiceInformation fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ServiceInformation>, Array] the matching ServiceInformation items; raises EarthquakeCatalogError on failure
