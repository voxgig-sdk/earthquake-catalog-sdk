...
module EarthquakeCatalogUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EarthquakeCatalogResponse.new({})
    ...
    if fetch_err
      ...
      response = EarthquakeCatalogResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EarthquakeCatalogResponse.new(fetched)
    else
