...
class EarthquakeCatalogSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = EarthquakeCatalogHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = EarthquakeCatalogHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, EarthquakeCatalogFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    EarthquakeCatalogUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = EarthquakeCatalogHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = EarthquakeCatalogHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => EarthquakeCatalogError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = EarthquakeCatalogHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = EarthquakeCatalogError.new(
        "graphql_error", "EarthquakeCatalogSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = EarthquakeCatalogSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
