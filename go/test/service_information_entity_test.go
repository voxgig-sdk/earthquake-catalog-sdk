...
func TestServiceInformationEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.ServiceInformation(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ServiceInformation(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set EARTHQUAKE_CATALOG_TEST_SERVICE_INFORMATION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		serviceInformationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.service_information")))
...
func service_informationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("EARTHQUAKE_CATALOG_TEST_SERVICE_INFORMATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["EARTHQUAKE_CATALOG_TEST_SERVICE_INFORMATION_ENTID"])
