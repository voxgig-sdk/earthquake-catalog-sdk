...
import (
	...
	omni "github.com/voxgig-sdk/earthquake-catalog-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EarthquakeCatalogError); is {
				out := map[string]any{
					"name":    "EarthquakeCatalogError",
...
func omniCtx(arg any, client *sdk.EarthquakeCatalogSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EarthquakeCatalogSDK); is {
			client = live
