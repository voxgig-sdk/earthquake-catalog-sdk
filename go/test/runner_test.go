...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("EARTHQUAKECATALOG_TEST_LIVE") == "TRUE" ||
		os.Getenv("EARTHQUAKECATALOG_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("EARTHQUAKECATALOG_TEST_EXPLAIN"); explain != "" {
