# EarthquakeCatalog Golang SDK Reference

Complete API reference for the EarthquakeCatalog Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *EarthquakeCatalogSDK`
...
#### `EarthquakeData(data map[string]any) EarthquakeCatalogEntity`

Create a new `EarthquakeData` entity instance. Pass `nil` for no initial data.

#### `ServiceInformation(data map[string]any) EarthquakeCatalogEntity`
...
```go
result, err := client.EarthquakeData(nil).Load(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
