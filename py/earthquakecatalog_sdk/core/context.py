...
from earthquakecatalog_sdk.core.helpers import get_ctx_prop, to_map
...
class EarthquakeCatalogContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = EarthquakeCatalogControl()
        ...
        if isinstance(sp, EarthquakeCatalogSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, EarthquakeCatalogResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, EarthquakeCatalogResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return EarthquakeCatalogOperation({})
    ...
    def make_error(self, code, msg):
        return EarthquakeCatalogError(code, msg, self)
