...
class EarthquakeDataEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EarthquakeDataEntity(self._client, opts)
    ...
    def data_get(self) -> EarthquakeData:
    ...
    def match_get(self) -> EarthquakeData:
    ...
    def load(self, reqmatch=None, ctrl=None) -> EarthquakeData:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.EarthquakeData().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[EarthquakeData]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.EarthquakeData().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
