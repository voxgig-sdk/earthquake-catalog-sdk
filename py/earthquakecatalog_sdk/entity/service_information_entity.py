...
from earthquakecatalog_sdk.earthquakecatalog_types import (
    ServiceInformation,
    ServiceInformationLoadMatch,
    ServiceInformationListMatch,
)
...
class ServiceInformationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ServiceInformationEntity(self._client, opts)
    ...
    def data_get(self) -> ServiceInformation:
    ...
    def match_get(self) -> ServiceInformation:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ServiceInformation:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ServiceInformation().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ServiceInformation]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ServiceInformation().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
