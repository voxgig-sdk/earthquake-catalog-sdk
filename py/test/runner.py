# EarthquakeCatalog SDK test runner
...
class EarthquakeCatalogTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EarthquakeCatalogTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EarthquakeCatalogTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EarthquakeCatalogTestRunner.getenv("EARTHQUAKE_CATALOG_TEST_LIVE")
        override = EarthquakeCatalogTestRunner.getenv("EARTHQUAKE_CATALOG_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EarthquakeCatalogTestRunner.getenv(key)
        ...
        explain = EarthquakeCatalogTestRunner.getenv("EARTHQUAKE_CATALOG_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if EarthquakeCatalogTestRunner._test_control is not None:
            return EarthquakeCatalogTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EarthquakeCatalogTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EarthquakeCatalogTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EarthquakeCatalogTestRunner.load_test_control()
...
def load_env_local():
    EarthquakeCatalogTestRunner.load_env_local()


def env_override(m):
    return EarthquakeCatalogTestRunner.env_override(m)


def entity_data(v):
    return EarthquakeCatalogTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EarthquakeCatalogTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EarthquakeCatalogTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EarthquakeCatalogTestRunner.load_test_control()


def live_client_options():
    return EarthquakeCatalogTestRunner.live_client_options()


def live_delay_ms():
    return EarthquakeCatalogTestRunner.live_delay_ms()
