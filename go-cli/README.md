# earthquake-catalog-cli

boru-driven command-line client **and** interactive REPL for the EarthquakeCatalog
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/earthquake-catalog-cli)
make build

# 2. See usage (words, entities, env vars)
./earthquake-catalog-cli --help

# 3. Provide credentials once, via the environment
export EARTHQUAKE_CATALOG_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./earthquake-catalog-cli list earthquake_data
./earthquake-catalog-cli load 1 earthquake_data            # {id:1} shorthand
./earthquake-catalog-cli load '{id:1}' earthquake_data       # explicit match map
./earthquake-catalog-cli list service_information

# 5. Override the API base URL for a single call
EARTHQUAKE_CATALOG_BASE=https://api.example.com ./earthquake-catalog-cli list earthquake_data

# 6. No arguments -> interactive REPL
./earthquake-catalog-cli
earthquake-catalog> list earthquake_data
earthquake-catalog> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/earthquake-catalog-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/earthquake-catalog-cli list earthquake_data
   ```
...
```sh
./earthquake-catalog-cli load 1 earthquake_data          # scalar shorthand for {id:1}
./earthquake-catalog-cli load '{id:1}' earthquake_data     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `earthquake-catalog>`). Each line is
evaluated as its own boru expression:

```text
$ ./earthquake-catalog-cli
earthquake-catalog> list earthquake_data
earthquake-catalog> /help
earthquake-catalog> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `earthquake_data`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/earthquake-catalog-cli`. |
...
(`./earthquake-catalog-cli <expr>`) and interactively (the REPL), and expressions compose the
